...
PH_HOLIDAYS = [
    "2026-01-01", "2026-02-17", "2026-02-25", "2026-04-02", "2026-04-03", "2026-04-09", 
...
async def index():
    ...
    today_type = get_day_type(today_str)
    ...
    if request.method == "POST":
        ...
        if get_day_type(log_date) in ["Weekend", "Holiday"]:
            return redirect(url_for("tracker.index"))
        ...
        credited_minutes = reg_m + raw_ot 
        
        log_data = {
            "user_id": user_id, "log_date": log_date, "am_in": am_in, "am_out": am_out, 
            "pm_in": pm_in, "pm_out": pm_out, "total_minutes": am_m + pm_m,
            "credited_minutes": credited_minutes, "ot_minutes": raw_ot
        }
    ...
    for rl in raw_logs:
        ...
        total_raw_m += (am_m + pm_m); total_credited_m += credited
        total_ot_m += (raw_ot if allow_ot else 0); am_total_m += am_m; pm_total_m += pm_m
        ...
        processed_logs.append({
            **rl, 'id': str(rl['_id']), 'credited_m': credited, 'total_m': am_m + pm_m,
            ...
            'month_key': log_dt.strftime('%B %Y'), 'raw_str': minutes_to_string(am_m + pm_m), 
            'credited_str': minutes_to_string(credited), 'am_str': f"{rl.get('am_in')} - {rl.get('am_out')}" if rl.get('am_in') else "-",
    ...
    return await render_template(
        ...
        ph_holidays=PH_HOLIDAYS, today_type=today_type
    )
